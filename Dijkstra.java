...
public class Dijkstra {
  ...
  public double computeEuclideanDistance(double ux, double uy, double vx, double vy) {
      // TODO
      double result = Math.sqrt((ux - vx) * (ux - vx) + (uy - vy) * (uy - vy));
  ...
  public void computeAllEuclideanDistances() {
      ...
      for (Vertex v : getVertices()) {
          ...
          for (Edge e : v.adjacentEdges) {
              ...
              double dist = computeEuclideanDistance(e.source.x, e.source.y, e.target.x, e.target.y);
              e.distance = dist;
          }
      }
  ...
  public void doDijkstra(String s) {
      ...
      //initialize a list for all vertices
      Collection<Vertex> vertexList = new ArrayList<Vertex>();
      vertexList = getVertices();
      //initialize list that will take unknown vertices before the for loop
      ...
      for (Vertex v : vertexList) {
          ...
          v.known = false;
          v.prev = null;
          //add to list of unknown vertices
          unknownVertexList.add(v);
          
      }
      
      u.distance = 0;//starting vertex which we start at 0
     
      
      while(unknownVertexList.isEmpty() != true) {
          
          double minDistance = Double.POSITIVE_INFINITY;
          Vertex smallestVertex = null;
          
          //this for loop will go through the adjacency list of the vertex we're on
          for (Vertex v : unknownVertexList) {
          ...
          for (Edge t : smallestVertex.adjacentEdges) {
              
              Vertex m = t.target;
              if (!m.known) {
                  
                  double edgeCost = t.distance;
                  
                  if (smallestVertex.distance + edgeCost < m.distance) {
                      
                      m.distance = smallestVertex.distance + edgeCost;
                      m.prev = smallestVertex;
                  }
              }
          }    
      }   
  ...
  public List<Edge> getDijkstraPath(String s, String t) {
      ...
      for (Vertex v = getVertex(t); v.prev != null; v = v.prev) {
          ...
          edges.addFirst(new Edge(v.prev, v, weight));
